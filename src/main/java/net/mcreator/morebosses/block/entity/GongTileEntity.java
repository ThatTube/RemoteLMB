...
public class GongTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public GongTileEntity(BlockPos pos, BlockState state) {
		super(MorebossesModBlockEntities.GONG.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(GongBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<GongTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<GongTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
