...
public class LaserEntity extends AbstractArrow implements ItemSupplier {
	...
	public LaserEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(MorebossesModEntities.LASER.get(), world);
	}
	...
	public void tick() {
		...
		// Se estava na água, restaurar a velocidade original
		if (this.isInWater()) {
			// Compensar o arrasto da água (a água normalmente reduz para 0.8x)
			this.setDeltaMovement(motionX * 1.25, motionY, motionZ * 1.25);
			
			// Também evitar que flutue/afunde muito
			if (Math.abs(motionY) < 0.01) {
				this.setDeltaMovement(motionX * 1.25, -0.01, motionZ * 1.25);
			}
		}
	...
	public static LaserEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		LaserEntity entityarrow = new LaserEntity(MorebossesModEntities.LASER.get(), entity, world);
	...
	public static LaserEntity shoot(LivingEntity entity, LivingEntity target) {
		LaserEntity entityarrow = new LaserEntity(MorebossesModEntities.LASER.get(), entity, entity.level());
