...
public class EyeOldEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(MorebossesModItems.OLD_EYE.get());

	public EyeOldEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(MorebossesModEntities.EYE_OLD.get(), world);
		setupEntity();
	}

	public EyeOldEntity(EntityType<? extends EyeOldEntity> type, Level world) {
		super(type, world);
		setupEntity();
	}

	public EyeOldEntity(EntityType<? extends EyeOldEntity> type, double x, double y, double z, Level world) {
		super(type, x, y, z, world);
		setupEntity();
	}

	public EyeOldEntity(EntityType<? extends EyeOldEntity> type, LivingEntity entity, Level world) {
	...
	private void discardWithEffects() {
		if (this.level().isClientSide) {
			...
			for (int i = 0; i < 15; ++i) {
				this.level().addParticle(ParticleTypes.GLOW, this.getX() + (this.random.nextDouble() - 0.5) * 0.5, this.getY() + (this.random.nextDouble() - 0.5) * 0.5, this.getZ() + (this.random.nextDouble() - 0.5) * 0.5,
	...
	public static EyeOldEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 1.0f, 0, 0);
	}

	public static EyeOldEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
		return shoot(world, entity, source, pullingPower * 1.0f, 0, 0);
	}

	public static EyeOldEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		EyeOldEntity entityarrow = new EyeOldEntity(MorebossesModEntities.EYE_OLD.get(), entity, world);
	...
	public static EyeOldEntity shoot(LivingEntity entity, LivingEntity target) {
		EyeOldEntity entityarrow = new EyeOldEntity(MorebossesModEntities.EYE_OLD.get(), entity, entity.level());
