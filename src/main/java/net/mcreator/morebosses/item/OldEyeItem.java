...
public class OldEyeItem extends Item {
	private final TagKey<Structure> structureTag;

	// Construtor com tag (para uso manual)
	public OldEyeItem(TagKey<Structure> structureTag) {
	...
	public OldEyeItem() {
		...
		this.structureTag = TagKey.create(net.minecraft.core.registries.Registries.STRUCTURE, new net.minecraft.resources.ResourceLocation("morebosses", "sanctuary"));
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!level.isClientSide()) {
			...
			if (blockpos != null) {
				...
				EyeOldEntity eyeEntity = new EyeOldEntity(MorebossesModEntities.EYE_OLD.get(), player.getX(), player.getY() + player.getEyeHeight() * 0.8, player.getZ(), level);
				...
				if (distance > 0) {
					eyeEntity.setDeltaMovement((dx / distance) * 0.5, (dy / distance + 1) * 0.5, // Leve curva para cima
							(dz / distance) * 0.5);
				}
